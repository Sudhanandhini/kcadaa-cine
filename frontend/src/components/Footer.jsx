import { Link } from 'react-router-dom';
import { useState } from 'react';
import logo from "../assets/images/logo1.jpg";

const Footer = () => {
  const [email, setEmail] = useState('');

  const handleSubscribe = (e) => {
    e.preventDefault();
    alert('Newsletter subscription feature coming soon!');
    setEmail('');
  };

  return (
    <footer className="bg-primary text-white">
      <div className="container-custom pt-12 pb-8">

        {/* 12-col grid for BOTH rows */}
        <div className="grid gap-10 md:gap-y-14 md:gap-x-10 md:grid-cols-12">

          {/* Row 1 - Logo (4 col) */}
          <div className="col-span-12 md:col-span-4 flex justify-center md:justify-start">
            <div className="w-[100px] h-[100px] flex items-center justify-center">
              <img src={logo} style={{ borderRadius: '50px' }} />
            </div>
          </div>

          {/* Row 1 - Newsletter (8 col) */}
          <div className="col-span-12 md:col-span-8 w-full md:text-left text-center">
            <h3 className="text-lg font-bold tracking-wide uppercase">
              OUR NEWSLETTER!
            </h3>

            {/* Yellow underline */}
            <div className="w-[12rem] h-[3px] bg-accent mt-1 mb-4 mx-auto md:mx-0" />

            <form
              onSubmit={handleSubscribe}
              className="w-full flex flex-col md:flex-row mt-12 md:mt-4"
            >
              <input
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="enter your email address...."
                className="flex-1 px-5 py-3 rounded-t md:rounded-t-none md:rounded-l text-gray-900 focus:outline-none"
                required
              />
              <button
                type="submit"
                className="bg-accent hover:bg-accent-dark text-white font-semibold px-10 py-3 rounded-b md:rounded-b-none md:rounded-r uppercase tracking-wide transition-colors"
              >
                Subscribe
              </button>
            </form>
          </div>

          {/* Row 2 - Explore (4 col) */}
          <div className="col-span-12 md:col-span-4 mt-8 md:mt-0">
            <h4 className="text-sm font-bold uppercase tracking-widest">
              Explore
            </h4>
            <div className="w-[8rem] h-[3px] bg-accent mt-2 mb-5" />
            <nav className="space-y-2">
              <Link to="/" className="block hover:text-accent transition-colors">
                Home
              </Link>
              <Link
                to="/about"
                className="block hover:text-accent transition-colors"
              >
                About Us
              </Link>
              <Link
                to="/members"
                className="block hover:text-accent transition-colors"
              >
                Members
              </Link>
              <Link
                to="/gallery"
                className="block hover:text-accent transition-colors"
              >
                Gallery
              </Link>
              <Link
                to="/events"
                className="block hover:text-accent transition-colors"
              >
                Event
              </Link>
              <Link
                to="/contact"
                className="block hover:text-accent transition-colors"
              >
                Contact
              </Link>
            </nav>
          </div>

          {/* Row 2 - Contact Info (4 col) */}
          <div className="col-span-12 md:col-span-4 mt-8 md:mt-0">
            <h4 className="text-sm font-bold uppercase tracking-widest">
              Contact Info
            </h4>
            <div className="w-[8rem] h-[3px] bg-accent mt-2 mb-5" />
            <div className="space-y-2 mt-12">
              <p>
                <a
                  href="tel:+917411041975"
                  className="hover:text-accent transition-colors"
                >
                  +91 74110 41975
                </a>
              </p>
              <p>
                <a
                  href="http://www.kcadaa.com"
                  className="hover:text-accent transition-colors"
                >
                  www.kcadaa.com
                </a>
              </p>
            </div>
          </div>

          {/* Row 2 - Social Media (4 col) */}
          <div className="col-span-12 md:col-span-4 mt-8 md:mt-0">
            <h4 className="text-sm font-bold uppercase tracking-widest">
              Social Media
            </h4>
            <div className="w-[8rem] h-[3px] bg-accent mt-2 mb-5" />
            <div className="flex space-x-4 mt-12">
              {/* Facebook */}
              <a
                href="#"
                className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-primary hover:bg-accent transition-colors"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>

              {/* Instagram */}
              <a
                href="#"
                className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-primary hover:bg-accent transition-colors"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              </a>

              {/* YouTube */}
              <a
                href="#"
                className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-primary hover:bg-accent transition-colors"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z" />
                </svg>
              </a>
            </div>
          </div>
        </div>

        {/* Bottom line + copyright */}
        <div className="mt-4 border-t border-white/30 pt-6">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <p className="text-xs text-center md:text-left">
              Copyright © 2025 Karnataka Cine Art Directors and Assistants association® |
              <a
                href="#"
                className="hover:text-accent transition-colors ml-1 font-medium"
              >
                Privacy Policy
              </a>
            </p>

            <p className="text-xs text-center md:text-right mt-2 md:mt-0">
              Developed by
              <a
                href="https://sunsys.in/"
                className="hover:text-accent transition-colors ml-1"
              >
                Sunsys Technologies
              </a>
            </p>
          </div>
        </div>

      </div>
    </footer>
  );
};

export default Footer;
